...
- **The effects module set** (2026-09, spatcore-only so far). Nine module types behind one
  ...
  phaser and the reverb have no prototype and are designed to the plan. The reverb's FDN model wraps
  ONE node of `reverb/ReverbFDNAlgorithm.h` per channel, which is why that class gained a
  delay-ceiling constructor argument and a node-index offset: without the offset every per-channel
  reverb would be node 0 and 32 of them would share one modal structure. **[V]**
- **The effects reverb: models, reflections, spillover** (2026-09, spatcore v0.4.0). The reverb
  module (`effects/modules/EffectReverbModule.h`) owns predelay, tone and mix; the tail is a model
  behind `reverb/ReverbTailModel.h`: the FDN node (models 0, and 2/3 which are reserved ids), a
  Dattorro plate (`PlateReverbModel.h`, model 1), a sixteen-line modulated hall
  (`ModulatedHallModel.h`, model 4) and the same hall with its four longest lines pitch-shifted
  (`ShimmerTap.h`, model 5, "Shimmer"). Every model decays by one law - three bands, gains
  `exp2(-9.9658 L / (sr RT60 mult))` per stretch of loop - so RT60, the multipliers and the
  crossovers mean the same everywhere, and every model is levelled to the FDN's wet (about -5 dB
  against the dry at 1.5 s, fully wet). Four early-reflection profiles
  (`EarlyReflections.h`, tables generated by `tools/reverb/gen_er_profiles.py` from an image-source
  model) sit in front of any model and push its input back by the profile's tail delay.
  Changes of topology do not fade the slot: the module keeps each tail class TWICE and runs up to
  three "worlds" (a tail plus its reflection pattern) at once - active, ringing, dying - with the
  input partitioned between them by the time each sample was WRITTEN (a 5 ms raised cosine), so a
  cue that changes the room lets the old tail ring out under the new one. All of it libm-free and
  allocation-free after `prepare()`; the tests pin each model's decay law, level, block-size
  invariance, key determinism, a max-settings run, and exact spillover partitions. **[V]**

  | per channel, 48 kHz | tail pair in memory | CPU (Core Ultra 7 255H laptop, one core, Release) |
  |---|---|---|
  | FDN (0) | ~430 KiB | 109 ns/sample, 0.52 % |
  ...
  48 kHz (54 MiB for 32 effects channels), doubling with the rate. A spillover runs two tails at
  once for as long as the old one rings (until -96 dBFS for 50 ms, or 30 s).
