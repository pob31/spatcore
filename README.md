...
| `ui/` | Shared GUI widgets: the interactive EQ curve display (ValueTree-backed, undoable), the per-band toggle, the audio-interface patch matrix (`ui/patch/`) and the dB send matrix (`ui/sends/`, schema-free: sources x destinations with a level and a switch per cell). Palette and localised strings are injected by the consuming app through provider callbacks, so spatcore stays app-agnostic |
