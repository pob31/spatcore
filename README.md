...
| `effects/` | Per-channel effects chain: the module contract (`IEffectModule` plus a `ModuleSlot` whose bypass and variant switches are click-free and whose NaN guard is not optional), the POD parameter set that crosses to the realtime side, and the chain-order vocabulary |
