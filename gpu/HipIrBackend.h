...
/*
    HipIrBackend — Linux/AMD twin of MetalIrBackend.

    Identical method surface and host-side behaviour (the shared work lives
    in IrConvHostState); only the device plumbing differs: hipRTC compiles
...
public:
    ...
    HipIrBackend& operator= (const HipIrBackend&) = delete;
...
using spatcore::gpu::HipIrBackend;
